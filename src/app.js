...
const isPrime = (number) =>{if(number<=1)return false; for(let i=2;i<number;i++) if(number%i==0){return false}; return true }
...
const multiply = (myArray) =>{return myArray.reduce((multiplied,number)=> multiplied*number)}

const sumOfSquares =(myArray) =>{return myArray.map(square).reduce((sum,number)=> sum+number) }